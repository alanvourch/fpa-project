...
- **fte_budget / fte_actual**: planned vs actual headcount. Budget is the
  ...
  group {sum(p['fte'] for p in gd.BU_PARAMS.values())}). Actual follows the 3-month smoothed payroll
...
def main():
    ...
    print(f"Group actual FTE range: {total_fte.min():.0f}..{total_fte.max():.0f} (plan {sum(p['fte'] for p in gd.BU_PARAMS.values())})")
