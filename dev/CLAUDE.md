...
- ~€100M revenue, 150 FTE, 4 client-facing business lines: Brand Events, Corporate Events,
  Digital/Influence, Government & Institutions (renamed 2026-07-20 from internal-function
  labels Production/Marketing/Digital/Back-Office — those aren't real revenue business
  lines at an events agency; see PROGRESS LOG)
...
**Last session:** 2026-07-20 — Sonnet 5 (recruiter-review pass: polish fixes + business-line rename). User asked for a critical read of the whole project as a finance recruiter, then worked through the findings live. Done, all 6 validators green after every change:
- **Narrative credential messaging:** `agents/narrative_agent.py` now catches `anthropic.AnthropicError` broadly (was missing the OAuth-refresh failure shape, which surfaced as a raw Python traceback in `output/pipeline_log.md`) and exits with a clean message. `orchestrator.py`'s board-pack wording and README's credentials section were reworded to drop "did not complete" framing and explain the client construction is provider-agnostic (personal Claude Pro subscription used for development; an internal model gateway or another approved provider is a config change, not a code change).
- **Precision/polish:** `variance_table.csv`, `forecast.csv`, and the cleaned CSV now round to cents at the serialization boundary only (internal math untouched). Removed the "video coming soon" line, fixed the dangling `AGENTS.md` -> `dev/AGENTS.md` reference (now points at `dev/CLAUDE.md`), replaced README's `<repo-url>` placeholder, clarified the showcase flow diagram's "Agent"/"Agents" node labels ("Agent · 1" / "Agents · 3").
- **Showcase UX:** report links now open in a new tab; the executive summary / variance / forecast report pages each got their matching chart inlined (via `showcase/build_site.py`, not the underlying `.md`, which stays chart-free prose by design); the "Material variances" and "History adjustments" tables (unreadable as wide table columns with an Explanation/Reason column) now render as a card list, plus "Download variance_table.csv / forecast.csv" links next to those reports.
- **Fixed a real bug found during the review:** `showcase/production_onepager.png` was a hand-taken screenshot that pre-dated a same-day fix, so it showed "No clear driver identified" for four variances the current PDF correctly labels as analyst input — directly undercutting the project's core provenance pitch. Replaced the static screenshot with a build-time render (`fitz`/PyMuPDF) straight from `output/bu_reports/brand_events.pdf`, so this class of staleness can't recur. Added `PyMuPDF==1.28.0` to requirements.txt.
- **Business-line rename (user-directed, simple-rename scope — see the AskUserQuestion in that conversation before re-litigating):** the 4 synthetic BUs were internal-function names (Production, Marketing, Digital, Back-Office) mislabeled as revenue business lines — Production/Marketing/Back-Office are support functions at a real events agency and don't carry revenue. Renamed to 4 client-facing verticals, same order/position in `BUS` (so RNG draws in `generate_dataset.py` land identically — every P&L figure is byte-identical, only labels changed): Production -> **Brand Events**, Marketing -> **Corporate Events**, Digital -> **Digital/Influence**, Back-Office -> **Government & Institutions**. Each keeps its full delivery P&L (revenue, COGS, payroll, opex) exactly as before — user chose simple rename over introducing separate non-revenue shared-service cost centers. Touched: `data/generate_dataset.py` (BUS, BU_PARAMS, anomaly injection incl. rewriting the Back-Office IT-failure story to an on-site AV/registration failure during a "ministry summit engagement" so it reads as that vertical's own delivery cost, not shared corporate IT), `data/generate_drivers.py` (also fixed a latent fragility: `build_projects` sorted rows alphabetically by BU name before drawing jitter, so a cosmetic rename silently reshuffled which month got which jitter — now sorts by `gd.BUS` list position, immune to renames), `agents/ingestion_agent.py` (CANONICAL_BUS + new plausible typo variants tested against `fuzzy_correct_bu` for confidence), `agents/bu_report_agent.py` (`slugify` now handles `/` and `&`), `data/business_notes.csv` + `data/analyst_commentary.csv` (BU column renamed, a couple of author roles and one Back-Office story reworded for vertical-specific coherence), all `tests/validate_*.py` (hardcoded BU-name string literals, plus two regex character classes in the ground-truth ANOMALY parsers that excluded `/` and `&` and were silently dropping the Digital/Influence and Government & Institutions anomalies from validation — a real latent parsing bug the rename exposed), `orchestrator.py` (BU display name in the board pack's BU-reports section now read from each report's own heading instead of reverse-guessed from the filename slug). Regenerated everything end to end (dataset, drivers, full pipeline, charts, showcase); hand-rewrote `output/executive_summary.md` (still the disclosed hand-written provenance version, no credentials available this session either) with the new names and the driver-figure changes that fell out of the reordering fix (Brand Events Nov-2025 ops corroboration is now 17 projects vs 17 planned, was 18 vs 17). Deleted stale `output/bu_reports/{production,marketing,digital,back_office}.{md,pdf}` left behind when the renamed files were generated alongside them.
**Next step:** the user may still want the showcase redeployed (gh-pages) with these changes, and film the demo video (still not done, no longer advertised as pending on the page). `dev/CLAUDE.md` §4 dataset description updated for the new business-line names.

**Earlier session (2026-07-10/11):** Fable 5 (v2 rebuild + user-feedback round). Full detail in PROGRESS.md; running decisions in NOTES.md — read both before changing anything.
**Feedback round (2026-07-11, after user review — supersedes anything below that conflicts):**
- **Analyst-input workflow (structural, user-required):** unexplained material variances now route to the FP&A analyst; explanations return via `data/analyst_commentary.csv` and render labeled "Analyst input (author, date)". Three provenance classes on every material row: documented note (4) / analyst input (14) / still open (2). Propagated through variance table (new `analyst_comment` column), BU one-pagers, charts (hatched = "routed to analyst"), QA (explanation-type check), validators, narrative prompt, hand-written exec summary (revalidated), README. Analyst comments are figure-free authored demo narrative (documented in README limitations + NOTES).
- **Showcase (user-required, now v3):** NO dark theme ever (finance/HR audience). v3 is a lighter, six-section redesign built for a 30-second recruiter scan with depth on demand (per the user's own numbered sketch): first-person business-partnering intro (ten years in FP&A, value is the time automation frees up), then 1 The data / 2 How it works (hand-built CSS flow diagram, SYSTEM vs ME colour-coded) / 3 The results (reports are the hero) / 4 Why this matters (four tiles, "the data" not "your data", + the EUR52M catch) / 5 See it run (real terminal transcript + one-pager) / 6 About (bio, LinkedIn linkedin.com/in/alan-vourch, email alan.vourch@gmail.com). Raw data lives behind a `<details>` expander and must only ever show verbatim strings from the dataset. Six generated reports are converted to styled same-origin HTML pages by `showcase/build_site.py` (deployable tree in gitignored `showcase/_site/`; deploy = copy _site to gh-pages branch, force-push); BU PDFs served same-origin so they OPEN IN TAB. Ultimate goal per user: recruiters project his added value into their company and want a meeting (selling himself, not a product).
- Hosting stays GitHub Pages on his domain (user confirmed, no Vercel).
