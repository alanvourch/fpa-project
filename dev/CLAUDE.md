...
- **Showcase v2 (user-required):** NO dark theme ever (finance/HR audience); copy first-person in Alan's voice, intro gives context up front (EUR100M, 4 business lines, 30 months, 10 documents); "who does what" section tags steps SYSTEM/ME; six generated reports converted to styled same-origin HTML pages by `showcase/build_site.py` (deployable tree in gitignored `showcase/_site/`; deploy = copy _site to gh-pages branch, force-push); BU PDFs served same-origin so they OPEN IN TAB (raw.githubusercontent would force downloads); closing CTA invites a walkthrough meeting via LinkedIn (linkedin.com/in/alan-vourch confirmed correct by user). Ultimate goal per user: recruiters must be able to project his added value into their company and want a meeting.
