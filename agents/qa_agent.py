...
def check_forecast_covers_all_flagged_rows(variance_table_text, forecast_text):
    ...
    if missing:
        return False, f"variance-flagged row(s) missing from the forecast's normalization audit trail: {missing}"
    return True, f"all {len(flagged)} variance-flagged row(s) are accounted for in the forecast's normalization audit trail"
