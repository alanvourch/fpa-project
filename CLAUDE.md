...
- Phase 6 (code complete; execution pending an API key): `agents/narrative_agent.py` — the ONE deliberate LLM call in this pipeline (`claude-sonnet-5`, `effort: medium`). Reads `output/variance_report.md` + `output/forecast_report.md`, system prompt enforces strict grounding (no invented figures/causes, preserve "no clear driver identified" honestly, cover the favorable variance prominently, no AI jargon). `tests/validate_narrative.py` is a hallucination check (no ground-truth table exists for free text): extracts every money figure/percentage from the generated `output/executive_summary.md` and confirms each traces to a real source figure within tolerance, plus checks the two honesty requirements. README.md's Architecture section now documents the deterministic-vs-LLM rationale + API key setup.
- Two real validator bugs caught via self-consistency sanity testing (before any LLM output existed): a percent regex with a `\b` right after the literal `%` that never matched anything, and an initial 3%-relative-only money tolerance that let a fabricated EUR9,999,999 "match" a real EUR9,901,455 at this company's multi-million-euro scale — fixed with a combined relative(≤2%) AND absolute(≤EUR15k) band.
- **`ANTHROPIC_API_KEY` is not set in this environment** (checked safely, existence only) and no `ant` CLI/OAuth profile is available — the agent has never actually been run. `output/executive_summary.md` does not exist yet.
**In progress:**
- Phase 6: code, validator, and README are done; blocked purely on running `agents/narrative_agent.py` with a real API key, then `tests/validate_narrative.py`, then reviewing/committing the generated file
**Next step:**
- Once `ANTHROPIC_API_KEY` is available: run the narrative agent, validate, review for quality, commit. Then Phase 7: QA/Reviewer Agent + Orchestrator (both Sonnet-appropriate — routine chaining/consistency logic, not a hard architecture call), then the recruiter-facing README polish, architecture diagram, and demo.
...
- Money-hallucination tolerance is a combined relative-AND-absolute band on purpose: relative-only fails at this company's scale (a fabricated EUR9,999,999 sat within 3% of a real EUR9.9M figure); absolute-only would fail on small opex lines if tuned for the big ones.
